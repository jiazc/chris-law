...
class Common extends Controller
{
     ...
     * 获取pg数据库的连接
     */
    protected function get_pgconn(){
        $host = config('DB_SDK')['hostname'];
        $port = config('DB_SDK')['hostport'];
        $dbname = config('DB_SDK')['database'];
        $user = config('DB_SDK')['username'];
        $password = config('DB_SDK')['password'];
        $conn_string  =  "host=".$host." port=".$port." dbname=".$dbname." user=".$user.
	" password=".$password;
        $pgconn = pg_connect($conn_string)or die("can't connect to database.");
        if( !$pgconn )
	{
            echo "create db connection failed";
            exit;
	}
	return $pgconn;
    }
