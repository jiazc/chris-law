        ...
        $.post($url, {x: val}, function (data) {
            if (data.code==1) {
                if (data.msg == '未审核') {
                    ...
                    var b = '<button class="btn btn-minier btn-yellow">已审核</button>';
...
/* 上线状态操作 */
$(function () {
	$('body').on('click','.online-btn',function () {
        var $url = this.href,
            val = $(this).data('id'),
            $btn=$(this);
        $.post($url, {x: val}, function (data) {
            if (data.code==1) {
                if (data.msg == '未上线') {
                    var a = '<button class="btn btn-minier btn-danger">未上线</button>';
                    $btn.children('div').html(a).attr('title','未上线');
                    return false;
                } else {
                    var b = '<button class="btn btn-minier btn-yellow">已上线</button>';
                    $btn.children('div').html(b).attr('title','已上线');
                    return false;
                }
            } else {
                layer.alert(data.msg, {icon: 5});
...
/* 反馈处理状态操作 */
$(function () {
	$('body').on('click','.do-ques-btn',function () {
        var $url = this.href,
            val = $(this).data('id'),
            $btn=$(this);
        $.post($url, {x: val}, function (data) {
            if (data.code==1) {
                if (data.msg == '处理中') {
                    var a = '<button class="btn btn-minier btn-warning">处理中</button>';
                    $btn.children('div').html(a).attr('title','处理中');
                    return false;
                }else if(data.msg == '已解决'){
                    var a = '<button class="btn btn-minier btn-success">已解决</button>';
                    $btn.children('div').html(a).attr('title','已解决');
                    return false;
                }else {
                    var b = '<button class="btn btn-minier btn-danger">驳回</button>';
                    $btn.children('div').html(b).attr('title','驳回');
                    return false;
                }
            } else {
                layer.alert(data.msg, {icon: 5});
...
/* 单图编辑上传 */
$("#edit_file0").change(function () {
    var objUrl = getObjectURL(this.files[0]);
    console.log("objUrl = " + objUrl);
    if (objUrl) {
        $("#edit_img").attr("src", objUrl);
    }
});
//
...
/* 媒体中心媒体分类编辑 */
$(function () {
	$('body').on('click','.sort-video-edit-btn',function () {
        var $url = this.href,
            val = $(this).data('id');
        $.post($url, {id: val}, function (data) {
            if (data.code == 1) {
                //$count = $("select[name=edit_label] option").size();
                var myselect=document.getElementById("edit_label");
                for($i=0;$i<100;$i++){
                    if(myselect.options[$i].value===data.category_id+""){
                ...
                $("#editsub_title").val(data.sub_title);
                $("#edit_url").val(data.url);               
                $("#edit_img").attr("src", data.img);
                $("#editorder_id").val(data.order_id);
            } else {
                layer.alert(data.msg, {icon: 5});
            }
        }, "json");
...
/* 法律条文预览 */
$(function () {
	$('body').on('click','.law-preview-btn',function () {
        var $url = this.href,
            val = $(this).data('id');
        $.post($url, {id: val}, function (data) {
            if (data.code == 1) {
                $("#myModaledit").show(300);
                $("#edit_law_title").text(data.law_title);
                $("#edit_law_detail").html(data.law_detail);
                $("#edit_key_word").text(data.key_word);
                $("#edit_id").val(data.id);
            } else {
                layer.alert(data.msg, {icon: 5});
            }
